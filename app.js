...
function renderStreak() {
  ...
  if (S.streak > 0) {
    ...
    badge.textContent = emoji + " " + S.streak + " day streak";
  } else {
...
function renderReview(pct) {
  ...
  // Unlock at 50% completion OR after 8 PM
  if (pct >= 50 || hour >= 20) {
    inner.className = "tasks-inner review-unlocked";
    if (hour >= 20 && pct < 50) {
      msg.textContent = "🌙 Evening unlock. Reflect on your day.";
    } else {
...
function pushToAndroid() {
  try {
    ...
    var sem3Left = S.mathSem3 ? S.mathSem3.filter(function(d) { return !d; }).length : 0;
    var tasksLeft = exLeft + sem3Left + (dailiesTotal - dailiesDone);
...
// === MINI GEMINI AI ===
const GEMINI_KEY = ""; // User must set their own key via Settings
let aiHistory = [];
...
async function sendAI() {
  ...
  try {
    ...
    var contextStr = "You are Rudranil's personal life AI assistant (not just workouts)." +
      "\n\n--- LIVE DASHBOARD CONTEXT ---" +
      "\nToday's productivity: " + todayPct + "%" +
      "\nStreak: " + (S.streak || 0) + " days" +
      "\nWorkout list: " + currentList +
      "\nToday's workout type: " + getTodayPPL() + " Day" +
      "\nSem 3 backlogs: " + sem3Done + "/" + MATH_SEM3_ALL.length + " done" +
      "\nSem 1 backlogs: " + sem1Done + "/" + MATH_SEM1_ALL.length + " done" +
      "\n4th Sem tasks: " + JSON.stringify(S.sem4) +
      "\nScratchpad: " + (S.scratchpad || "empty") +
      "\nLast review: " + (S.review || "none") +
      "\n--- END CONTEXT ---" +
      ...
      "\nOnly output JSON if modifying state. Keep regular text replies concise (2-3 sentences max).";
...
function pruneHistory() {
  ...
  keys.sort(function(a, b) {
    var partsA = a.replace('rd-v7-', '').split('-').map(Number);
    var partsB = b.replace('rd-v7-', '').split('-').map(Number);
    var dA = new Date(partsA[0], partsA[1] - 1, partsA[2]);
    var dB = new Date(partsB[0], partsB[1] - 1, partsB[2]);
    return dA - dB;
  });
  var toRemove = keys.length - 90;
  for (var i = 0; i < toRemove; i++) {
    delete S.history[keys[i]];
  }
  console.log("[History] Pruned " + toRemove + " old entries. Keeping 90 days.");
}
